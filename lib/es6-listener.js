...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
    ...
    module.exports.invoke = function(options, callback) {
        //apply defaults
        options = options || {};
        fs.readFile(path.resolve(__dirname, "es6-listener/code.js.template"), "utf8",function(err, result) {
            if (err) { return callback(err); }
            if (typeof options.name !== 'string') {
                return callback(new Error("Invalid Argument. Expected string."));
            }
            var name = options.name;
            result = result.replace(/\${Listener}/g, name);
            //append file
            var file = S(name).dasherize().replace(/^-/,"") + "-listener.es6";
            fs.exists(path.join(process.cwd(),"app/controllers", file), function(exists) {
                ...
                mkdirp(path.join(process.cwd(),"app/controllers"), function(err) {
                    if (err) { return callback(err); }
